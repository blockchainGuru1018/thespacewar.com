...
class MatchComService {
    ...
    _onSnapshot(snapshot) {
        ...
        for (const player of this.getPlayers()) {
            ...
            const data = {
                ...
                ...preparePlayerState(
                    this._getPlayerChangedState(playerId, snapshot)
                ),
    ...
    _endGame() {
        const [
            firstPlayerId,
            secondPlayerId,
        ] = this._matchService.getPlayerOrder();
    ...
    _somePlayerHasLost() {
        ...
        return (
            this._playerHasLost(firstPlayerId) ||
            this._playerHasLost(secondPlayerId)
        );
    }
    ...
    _playerHasLost(playerId) {
        ...
        const cannotCounterOrHasAlreadyCountered =
            !this._playerCanAvoidStationCardAttack(playerId) ||
