...
module.exports = function PlayerInactivityService(
    ...
    function validateTimestampsForPlayer(playerId, playerState, gameStartTime) {
        if (playerState && playerState.phase !== PHASES.wait) {
            if (playerAsBeenInactiveForMaxPeriod(playerState.events, gameStartTime)) {
                logger.log(`player ${playerId} was forced to retreat because of inactivity`);
                retreat(playerId);
            }
        }
    }
    ...
    function playerAsBeenInactiveForMaxPeriod(events, gameStartTime) {
        ...
        const lastEventOrGameStartTimeStamp = events.length > 0 ? events[events.length - 1].created : gameStartTime;
        return maxMinuteOfInactivity <= currentTimestamp - lastEventOrGameStartTimeStamp;
    }


    function isOpponentDisconnected() {
        return matchComService.getPlayerConnection(matchService.getOpponentId()).disconnected;
    }
