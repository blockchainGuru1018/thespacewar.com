...
function PutDownExcellentWork({ playerServiceProvider, playerServiceFactory }) {
    return {
        forPlayer,
    };
    ...
    function forPlayer(playerId, cardData, { choice = "" } = {}) {
        ...
        if (choice === "draw") {
            ...
            addRequirementFromSpec.forCardAndChoiceOfRequirement(
                cardData,
                choice
            );
        } else {
            throw new Error(
                "Excellent work requires that the player makes a choice, no choice or an invalid choice was provided"
            );
        }
    }
}
