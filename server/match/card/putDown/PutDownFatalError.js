...
function PutDownFatalError({
    ...
    function canBePlayed(playerId, cardData, { choice: targetCardId } = {}) {
        ...
        const actionPoints = playerServiceFactory.playerActionPointsCalculator(playerId).calculate();

        return fatalError.actionWhenPutDownInHomeZone.validTarget(target, actionPoints);
    }
    ...
    function targetCard(playerId, targetCardId) {
        const opponentId = matchService.getOpponentId(playerId);
        const targetCardData = playerServiceFactory.playerStateService(opponentId).findCardFromZonesOrStation(targetCardId);
        return playerServiceFactory.cardFactory().createCardForPlayer(targetCardData, opponentId);
    }
