...
function PutDownCardController(deps) {
    ...
    function counterCard(playerId, { cardId, targetCardId }) {
        ...
        if (turnControl.opponentHasControlOfPlayersTurn()) {
            const opponentTurnControl = playerServiceFactory.turnControl(
                opponentId
            );
    ...
    function validateIfCanProgressCounterCardRequirementByCount(
        count,
        playerId
    ) {
    ...
    function checkIfCanPutDownCard({ playerId, location, cardData, choice }) {
        ...
        const nameOfCardSource = playerStateService.nameOfCardSource(
            cardData.id
        );
        if (nameOfCardSource === location)
        ...
        if (location === "zone") {
            ...
            if (cardCanBePlayedChecker.hasCheckerForCard(cardData)) {
                if (
                    !cardCanBePlayedChecker.canBePlayed(playerId, cardData, {
                        choice,
                    })
                ) {
                    throw new CheatError("Cannot put down card");
