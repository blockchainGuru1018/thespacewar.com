...
module.exports = function ({
    ...
    function onSelectCard(playerId, { cardGroups }) {
        ...
        if (destroyCardFromUseOfDormantEffect(requirement)) {
            logUseOfDormantEffect(
                playerId,
                requirement.usedDormantEffect.cardId
            );
        }
    ...
    function getCost({ cardGroups, playerId }) {
        ...
        const totalCost = allCardInGroups.reduce(
            (acc, cardId) =>
                acc +
                    (playerStateService.findCardFromAnySource(cardId) || {})
                        .cost || 0,
            0
        );
        return totalCost;
    ...
    function moveCardsToHomeZone(cardGroups, playerId) {
        ...
        for (const group of cardGroups) {
            for (const cardId of group.cardIds) {
                const cardData = removeCardFromSource(
                    cardId,
                    group.source,
                    playerId
                );
    ...
    function logUseOfDormantEffect(playerId, triggerCardId) {
        ...
        const triggerCard = playerStateService.findCardFromAnySource(
            triggerCardId
        );
    ...
    function getSelectedCardsCount(cardGroups) {
        return cardGroups.reduce(
            (total, group) => total + group.cardIds.length,
            0
        );
    }
};
