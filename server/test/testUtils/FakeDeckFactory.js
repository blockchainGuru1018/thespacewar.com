...
FakeDeckFactory.fromCards = cards => {
    const cardDataAssembler = FakeCardDataAssembler({ createAll: () => [...cards] });
    return FakeDeckFactory({ cardDataAssembler });
};

FakeDeckFactory.createDeckFromCards = cards => {
    const cardDataAssembler = FakeCardDataAssembler({ createAll: () => cards.map(c => FakeCardDataAssembler.createCard(c)) });
