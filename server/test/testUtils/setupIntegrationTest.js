...
module.exports = function setup(
    state,
    { playerId = "P1A", opponentId = "P2A", matchDeps = {} } = {}
) {
    const firstPlayerConnection = FakeConnection(["stateChanged"]);
    const secondPlayerConnection = FakeConnection(["stateChanged"]);
    const players = [
        Player(playerId, firstPlayerConnection),
        Player(opponentId, secondPlayerConnection),
    ];
    const match = createMatch({ players, ...matchDeps });
    const firstPlayerAsserter = StateAsserter(
        match,
        firstPlayerConnection,
        playerId
    );
    const secondPlayerAsserter = StateAsserter(
        match,
