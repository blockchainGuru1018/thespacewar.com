...
module.exports = {
  "when in draw phase and has 1 card in station draw-row": {
    ...
    "should emit drawCards to first player"() {
      ...
      assert.calledWith(
        this.firstPlayerConnection.drawCards,
        sinon.match({
  ...
  "when discard opponent top 2 cards and has more cards to draw": {
    async setUp() {
      ...
      this.match = createMatch({
        players,
        gameConfig: GameConfig({ millCardCount: 2 }),
      });
      this.match.restoreFromState(
        createState({
          playerStateById: {
    ...
    "should emit drawCards to first player"() {
      ...
      assert.calledWith(
        this.firstPlayerConnection.drawCards,
        sinon.match({ moreCardsCanBeDrawn: true })
      );
    },
    ...
    "should emit state changed with requirement removed"() {
      ...
      assert.calledWith(
        ...
        sinon.match({
          ...
          events: sinon.match.some(
            sinon.match({ type: "drawCard", byEvent: true })
          ),
          requirements: [],
        })
      );
  ...
  "when has draw card requirement with count 3 and skip draw card": {
    ...
    "should NOT emit state changed with cards drawn"() {
      refute.defined(
        this.firstPlayerConnection.stateChanged.lastCall.args[0].cardsOnHand
      );
    },
  },
  ...
  "when has COMMON draw card requirement with count 1 and draw card and second player is NOT waiting": {
    ...
    "should emit state changed with requirement updated to first player"() {
      ...
      assert.calledWith(
        ...
        sinon.match({
          requirements: [
            { type: "drawCard", count: 0, common: true, waiting: true },
          ],
        })
      );
    },
