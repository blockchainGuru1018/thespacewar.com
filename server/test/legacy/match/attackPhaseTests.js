...
module.exports = {
    "when first player is in attack phase and moves card": {
        async setUp() {
            ...
            this.secondPlayerConnection = FakeConnection2([
                "opponentMovedCard",
                "stateChanged",
            ]);
            const players = [
        ...
        "should emit opponentMovedCard"() {
            ...
            assert.calledWith(
                this.secondPlayerConnection.opponentMovedCard,
                "C1A"
            );
        },
        "when restore state for first player should NOT have moved card in own zone"() {
            this.match.refresh("P1A");
            const state = this.firstPlayerConnection.stateChanged.lastCall
                .args[0];
        ...
        "when restore state for first player should have a move card event"() {
            ...
            const state = this.firstPlayerConnection.stateChanged.lastCall
                .args[0];
            const moveCardEvents = state.events.filter(
                (e) => e.type === "moveCard"
            );
        ...
        "when restore state for second player should have card in opponentCardsInPlayerZone"() {
            ...
            const state = this.secondPlayerConnection.stateChanged.lastCall
                .args[0];
            ...
            createState({
                ...
                playerStateById: {
                    P1A: {
                        ...
                        cardsInZone: [createCard({ id: "C1A" })],
                        events: [
                            { type: "putDownCard", cardId: "C1A", turn: 1 },
                        ],
                    },
                },
            })
    ...
    "when first player attack card in opponents own zone": {
        ...
        "when second player restore state should have damaged card still in own zone"() {
            ...
            const {
                cardsInZone,
            } = this.secondPlayerConnection.stateChanged.lastCall.args[0];
        ...
        "should emit opponentAttackedCard with card ids and damage amount"() {
            assert.calledWith(
                this.secondPlayerConnection.opponentAttackedCard,
                sinon.match({
                    attackerCardId: "C1A",
                    defenderCardId: "C2A",
                    newDamage: 1,
    ...
    "when first player attack card in own zone from opponents zone": {
        async setUp() {
            ...
            this.match = createMatch({
                players: [Player("P1A"), Player("P2A", this.connection)],
            });
            this.match.restoreFromState(
                createState({
                    turn: 2,
        ...
        "when second player restore state should have card undamaged and still in own zone"() {
            ...
            const {
                cardsInOpponentZone,
            } = this.connection.stateChanged.lastCall.args[0];
    ...
    "when defender has 2 in defense and 1 in damage and first player attacks with card with 1 in attack": {
        async setUp() {
            this.connection = FakeConnection2([
                "stateChanged",
                "opponentAttackedCard",
            ]);
            this.match = createMatch({
        ...
        "should emit opponent attacked card and that defender card was destroyed"() {
            assert.calledWith(
                this.connection.opponentAttackedCard,
                sinon.match({
                    attackerCardId: "C1A",
                    defenderCardId: "C2A",
                    defenderCardWasDestroyed: true,
                ...
                createState({
                    ...
                    playerStateById: {
                        P1A: {
                            phase: "attack",
                            cardsInZone: [
                                createCard({ id: "C1A", type: "defense" }),
                            ],
                        },
                    },
                })
    ...
    "missile cards:": {
                    ...
                    createState({
                        ...
                        playerStateById: {
                            ...
                            P2A: {
                                ...
                                events: [
                                    MoveCardEvent({ turn: 1, cardId: "C2A" }),
                                ],
                            },
                        },
                    })
        ...
        "when missile card attack station on the same turn it moved to zone": {
            ...
            "should emit flipped station card to first player": function () {
                ...
                const flippedCards = opponentStationCards.filter(
                    (s) => s.flipped
                );
                assert.equals(flippedCards.length, 1);
                assert.match(flippedCards[0].card, { id: "C2A" });
    ...
    "station cards:": {
        ...
        "when card has NOT been in enemy zone for 1 turn": {
            setUp() {
                ...
                this.error = catchError(() =>
                    this.match.attackStationCard("P1A", attackOptions)
                );
            },
            "should throw error"() {
                assert(this.error);
        ...
        "when attacking station card that does NOT exist": {
            setUp() {
                ...
                const attackOptions = {
                    attackerCardId: "C1A",
                    targetStationCardIds: ["NO_CARD_HAS_THIS_ID"],
                };
                this.error = catchError(() =>
                    this.match.attackStationCard("P1A", attackOptions)
                );
            ...
            "should NOT emit flipped station card to first player"() {
                refute.called(
                    this.firstPlayerConnection.opponentStationCardsChanged
                );
            },
        },
        "when attacking station card that is flipped": {
            ...
            "should throw error"() {
                ...
                assert.equals(
                    this.error.message,
                    "Cannot attack a flipped station card"
                );
            },
            "should NOT emit flipped station card to second player"() {
                refute.called(this.secondPlayerConnection.stationCardsChanged);
        ...
        "when enemy has 2 station cards and attacker has 2 in attack but only gives 1 target station card id": {
            ...
            "should throw error"() {
                ...
                assert.equals(
                    this.error.message,
                    "Need more target station cards to attack"
                );
            },
        },
    ...
    "sacrifice:": {
        ...
        "when it is NOT attack phase and thus card cannot sacrifice": {
            async setUp() {
                ...
                this.error = catchError(() =>
                    this.match.sacrifice("P1A", options)
                );
            },
            "should throw error"() {
                assert(this.error);
    ...
    "sacrifice and target station": {
                    ...
                    sinon.match({
                        ...
                        stationCards: [
                            sinon.match({
                                id: "C2A",
                                flipped: true,
                                card: sinon.match({ id: "C2A" }),
                            }),
                            sinon.match({
                                id: "C3A",
                                flipped: true,
                                card: sinon.match({ id: "C3A" }),
                            }),
                            sinon.match({
                                id: "C4A",
                                flipped: true,
                                card: sinon.match({ id: "C4A" }),
                            }),
                            sinon.match({
                                id: "C5A",
                                flipped: true,
                                card: sinon.match({ id: "C5A" }),
                            }),
                        ],
                    })
                );
        ...
        "when sacrifice card against 4 target station cards and there are 4 left but 1 target card id is NOT valid": {
            async setUp() {
                ...
                const options = {
                    cardId: "C1A",
                    targetCardIds: ["C2A", "C3A", "C4A", "NOT_VALID_ID"],
                };
                this.error = catchError(() =>
                    this.match.sacrifice("P1A", options)
                );
        ...
        "when sacrifice card against 5 target station cards": {
            async setUp() {
                ...
                const options = {
                    cardId: "C1A",
                    targetCardIds: ["C2A", "C3A", "C4A", "C5A", "C6A"],
                };
                this.error = catchError(() =>
                    this.match.sacrifice("P1A", options)
                );
    ...
    "when move repair ship to opponent zone and back home again and repair flipped station card": {
        async setUp() {
                    ...
                    playerStateById: {
                        P1A: {
                            ...
                            cardsInOpponentZone: [
                                createCard({
                                    id: "C1A",
                                    commonId: SmallRepairShip.CommonId,
                                }),
                            ],
                            stationCards: [
            ...
            this.error = catchError(() =>
                this.match.repairCard("P1A", options)
            );
        },
        "should NOT throw an error"() {
            refute(this.error);
        ...
        "station card should be unflipped"() {
            assert.calledWith(
                this.firstPlayerConnection.stateChanged,
                sinon.match({
                    stationCards: sinon.match.some(
                        sinon.match({ id: "S1A", flipped: undefined })
                    ),
                })
            );
        },
