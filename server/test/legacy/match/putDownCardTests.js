...
module.exports = {
    ...
    'Supernova:': {
        ...
        'when first player put down Supernova and both players has NO station cards': {
            ...
            'should Throw Cannot put down card error of type CheatDetected'() {
                const error = catchError(() => this.match.putDownCard('P1A', {location: 'zone', cardId: 'C3A'}));
                assert.equals(error.message, 'Cannot put down card');
                assert.equals(error.type, 'CheatDetected');
            },
        ...
        'when first player put down Supernova and both players has 1 flipped station card and 1 unflipped station card': {
            ...
            'should Throw Cannot put down card error of type CheatDetected'() {
                const error = catchError(() => this.match.putDownCard('P1A', {location: 'zone', cardId: 'C1A'}));
                assert.equals(error.message, 'Cannot put down card');
                assert.equals(error.type, 'CheatDetected');
            },
        }
    ...
    'Fatal Error:': {
            ...
            setUp() {
                ...
                this.match.restoreFromState(createState({
                    playerStateById: {
                        ...
                        'P2A': {
                            cardsInOpponentZone: [createCard({id: 'C2A', commonId: 'C2B'})],
                            cardsInDeck: [
                                createCard({id: 'C3A'}),
                                createCard({id: 'C4A'})
                            ]
        ...
        'when put down Fatal Error with invalid card id as choice': {
            setUp() {
                ...
                this.error = catchError(() => this.match.putDownCard('P1A', {
                    location: 'zone',
                    cardId: 'C1A',
                    choice: null
                }));
    ...
    'when put down Missiles launched': {
        async setUp() {
            this.firstPlayerConnection = FakeConnection2(['stateChanged']);
            const players = [Player('P1A', this.firstPlayerConnection)];
            this.match = createMatch({players}, [{id: MissilesLaunched.CommonId, price: '1'}]);
            this.match.restoreFromState(createState({
                turn: 1,
                playerStateById: {
                    'P1A': {
