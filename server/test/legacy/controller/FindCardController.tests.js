...
module.exports = {
    "when has selected 2 cards": {
        setUp() {
            ...
            const controller = Controller({
                playerRequirementUpdaterFactory: this
        ...
        "should progress requirement count"() {
            assert.calledWith(
                this.playerRequirementUpdater.progressRequirementByCount,
                2
            );
        },
        "should remove card from deck"() {
            ...
            assert.calledWith(
                this.playerStateService.removeCardFromDeck,
                "C1A"
            );
        },
        "should remove card from discard pile"() {
            assert.calledOnce(
                this.playerStateService.removeCardFromDiscardPile
            );
            assert.calledWith(
                this.playerStateService.removeCardFromDiscardPile,
                "C2A"
        ...
        "should add cards to home zone"() {
            ...
            assert.calledWith(
                this.playerStateService.putDownCardInZone,
                sinon.match({ id: "C1A" }),
        ...
        "should create requirement updater correctly"() {
            assert.calledWith(
                this.playerRequirementUpdaterFactory.create,
                PlayerId,
                { type: "findCard" }
            );
        },
    ...
    "checks if can progress requirement correctly": function () {
        ...
        controller.onSelectCard(PlayerId, {
            cardGroups: [{ source: "deck", cardIds: ["C1A"] }],
        });

        assert.calledWith(
            playerRequirementUpdater.canProgressRequirementByCount,
            1
        );
    },
        ...
        "when can NOT progress requirement": {
            async setUp() {
                ...
                this.error = catchError(() =>
                    controller.onSelectCard(PlayerId, options)
                );
            },
            "should throw"() {
                assert(this.error);
                assert.equals(
                    this.error.message,
                    "Cannot select more cards than required"
                );
            },
    ...
    "when select 1 card from action station cards and target is hand": {
        ...
        "should add card to home zone"() {
            ...
            assert.calledWith(
                this.playerStateService.addCardToHand,
                sinon.match({ id: "C1A" })
            );
        },
    },
    "when select card in opponents home zone and target is opponent discard pile": {
        ...
        "should discard opponent card"() {
            ...
            assert.calledWith(
                this.opponentStateService.discardCard,
                sinon.match({ id: "C2A" })
            );
        },
        "should remove opponent card from opponents home zone"() {
            ...
            assert.calledWith(
                this.opponentStateService.removeCardFromHomeZone,
                "C2A"
            );
        },
    },
    "when requirement is card used dormant effect and is set to be destroyed": {
        setUp() {
            this.playerStateService = fakePlayerStateServiceFactory.withStubs({
                ...
                removeCardFromAnySource: stub().returns({
                    id: "C1A",
                    commonId: "C1B",
                }),
                discardCard: stub(),
            ...
            const controller = Controller({
                playerServiceProvider: FakePlayerServiceProvider({
                    playerStateService: this.playerStateService,
                    opponentStateService: fakePlayerStateServiceFactory.withStubs(
        ...
        "should remove triggered card from player"() {
            ...
            assert.calledWith(
                this.playerStateService.removeCardFromAnySource,
                "C1A"
            );
        },
        "should discard players triggered card"() {
            ...
            assert.calledWith(
                this.playerStateService.discardCard,
                sinon.match({ id: "C1A" })
            );
        },
        "should append to action log"() {
            ...
            assert.calledWith(
                this.opponentActionLog.opponentTriggeredCard,
                sinon.match({
                    id: "C1A",
                    commonId: "C1B",
                })
...
function FakePlayerServiceProvider({
    playerStateService,
