module.exports = function ({
  ...
  async function onRegisterConnection({ secret, userId }) {
    if (!securityController.isAuthorized(secret, userId)) {
      logger.log(
        "Unauthorized user performing action on match",
        "authorization"
      );
      return;
    }
    ...
    if (ongoingMatch) {
      try {
        ...
        logger.log(
          "Error when registering connection for user: " + error.message,
          "reconnect error"
        );
  ...
  async function onReconnectBot({ secret, userId }) {
    ...
    if (ongoingMatch) {
      try {
        ...
        logger.log(
          "Error when reconnecting bot: " + error.message,
          "reconnect error"
        );
        logger.log("Raw error: " + error, "reconnect error");
      }
  ...
  async function onMatchMessage(data) {
    ...
    try {
      ...
      const errorMessage = `(${new Date().toISOString()}) Error in action to match: ${
        error.message
      } - DATA: ${dataString} - RAW ERROR: ${rawErrorMessage}`;
  ...
  function isSocketMessageAuthorized(data) {
    const playerId =
      data.playerId === BotId ? data.playerIdControllerBot : data.playerId;
