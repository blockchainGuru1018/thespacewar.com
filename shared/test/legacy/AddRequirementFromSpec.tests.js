...
module.exports = {
    'spec with property "ifAddedAddAlso" and requirement is successfully added': {
        setUp() {
            ...
            testHelper.stub('playerRequirementService', 'P1A', this.playerRequirementService);
            testHelper.stub('playerRequirementService', 'P2A', this.opponentRequirementService);
            this.addRequirementFromSpec = testHelper.addRequirementFromSpec('P1A');
        ...
        'should add requirement to opponent'() {
            assert.calledOnce(this.opponentRequirementService.addCardRequirement);
            assert.calledWith(this.opponentRequirementService.addCardRequirement, sinon.match({
                type: 'drawCard',
                count: 1,
                common: true
            }));
        ...
        'should add extra requirement to player'() {
            assert.calledOnce(this.playerRequirementService.addEmptyCommonWaitingRequirement);
            assert.calledWith(this.playerRequirementService.addEmptyCommonWaitingRequirement, sinon.match({ type: 'drawCard' }));
        }
    },
