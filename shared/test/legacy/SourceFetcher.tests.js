...
module.exports = {
  opponentAny: {
    asd() {
      ...
      const triggerCard = {
        canCounterCard: (otherCard) => otherCard.id === "C1A",
      };

      const cards = sourceFetcher.opponentAny(
        { canBeCountered: true },
        { triggerCard }
      );
  ...
  "can get draw station cards": function () {
    const testHelper = TestHelper(
      createState({
        ...
        playerStateById: {
          P1A: {
            stationCards: [{ place: "draw", card: createCard({ id: "C1A" }) }],
          },
        },
      })
    );
  ...
  "can exclude card by id": function () {
    const testHelper = TestHelper(
      createState({
        ...
        playerStateById: {
          P1A: {
            discardedCards: [
              createCard({ id: "NOT_FILTERED_CARD" }),
              createCard({ id: "FILTERED_CARD" }),
            ],
          },
        },
      })
    ...
    const cards = sourceFetcher.discardPile({
      excludeCardIds: ["NOT_FILTERED_CARD"],
    });
