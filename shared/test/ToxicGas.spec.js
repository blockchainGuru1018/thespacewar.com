...
describe('Toxic gas behavior', () => {
    ...
    it('should be able to attack shields',()=>{
        const someShieldCard = FakeCardDataAssembler.createCard({ commonId: '21', stopsStationAttack: () => true}); // Energy Shield
        expect(toxicGas.canAttackCard(someShieldCard)).toBe(true);
    });
    it('should NOT be able to attack no-shield cards',()=>{
        const someCard = FakeCardDataAssembler.createCard({ commonId: 'C1', stopsStationAttack: () => false});
