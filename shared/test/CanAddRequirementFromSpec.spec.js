...
test('when requirement spec has an "onlyWhen" condition for opponent whose action returns false, can NOT add requirement', () => {
    ...
    const canAddRequirementFromSpec = setupClassWithRequirementConditions(requirementConditions(onlyWhenStub));
    const spec = withSpecForOpponent({ onlyWhen: 'targetIsFlippedStationCard' });
    const target = 'C1A';

    const result = canAddRequirementFromSpec.forCardWithSpecAndTarget({}, spec, target);
...
test('when checking "onlyWhen" condition for OPPONENT should provide condition name and target data', () => {
    ...
    expect(onlyWhenStub).toBeCalledWith('targetIsFlippedStationCard', { card, target, specForPlayer: specForOpponent });
});
...
test('when checking "onlyWhen" condition for PLAYER should provide condition name and target data', () => {
    ...
    expect(onlyWhenStub).toBeCalledWith('targetIsFlippedStationCard', { card, target, specForPlayer });
});
...
test('when has NO "onlyWhen" condition should be able to add requirement', () => {
    ...
    const result = canAddRequirementFromSpec.forCardWithSpecAndTarget(card, spec, target);
...
test('when spec has no special condition can add requirement', () => {
    ...
    expect(canAddRequirementFromSpec.forCardPutDownInHomeZone({}, { forOpponent: [] })).toBe(true);
});
