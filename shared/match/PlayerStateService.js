...
class PlayerStateService {
  ...
  findCardFromHand(cardId) {
    return (
      this.getPlayerState().cardsOnHand.find((c) => c.id === cardId) || null
    );
  }
  ...
  moveCard(cardId) {
    ...
    if (cardZoneIndex >= 0) {
      ...
      const turn = this._matchService.getTurn();
      this.storeEvent(
        MoveCardEvent({ turn, cardId, cardCommonId: updatedCardCommonId })
      );
    } else {
  ...
  updateStationCard(cardId, updateFn) {
    if (!this.findStationCard(cardId))
      throw Error(
        "Could not find station card when trying to update it. ID: " + cardId
      );
