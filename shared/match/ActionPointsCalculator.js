...
module.exports = function ({ cardInfoRepository }) {
    ...
    function calculate({ phase, turn, events, actionStationCardsCount }) {
        ...
        const playerIsPastActionPhase = PHASES_AFTER_ACTION_PHASE.includes(
            phase
        );
        if (playerIsPastActionPhase) return actionPoints;

        actionPoints -= getCostOfDurationCardsLeftFromPreviousTurns(
            events,
            turn
        );

        const eventsThisTurn = events.filter(
            (e) => e.turn === turn && !e.grantedForFreeByEvent
        );
        for (let event of eventsThisTurn) {
            if (event.type === "putDownCard") {
                ...
                if (
                    event.location === "station-action" &&
                    !event.startingStation
                ) {
                    actionPoints -= 2;
                }
                ...
                const costOfTargetCard = getCostOfCard(
                    event.targetCardCommonId
                );
                actionPoints -= costOfTargetCard;
            }
        }

        const moveStationCardEventsThisTurn = eventsThisTurn.filter(
            (e) => e.type === "moveStationCard"
        );
        actionPoints += actionPointsToAddBecauseOfStationCardMoveEvents(
            moveStationCardEventsThisTurn
        );

        const cheatPoints = events.reduce(
            (acc, e) =>
                e.type === "cheatAddActionPoints" ? acc + e.count : acc,
            0
        );
    ...
    function getCostOfDurationCardsLeftFromPreviousTurns(events, turn) {
        ...
        for (const event of events) {
            if (eventIsPutDownDurationCardInZone(event, turn)) {
                ...
                event.type === "discardCard" &&
                durationCardsInPlay.has(event.cardId)
            ) {
                totalCost -= cardInfoRepository.getCost(event.cardCommonId);
            }
        }
    ...
    function shouldReducePointsByCostOfCard(putDownCardEventFromCurrentTurn) {
        return (
            putDownCardEventFromCurrentTurn.location === "zone" ||
