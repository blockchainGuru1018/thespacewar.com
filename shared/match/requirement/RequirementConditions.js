module.exports = function RequirementConditions({
    ...
    function onlyWhen(names, { card, specForPlayer, target }) {
        return conditionsTypeAndNames('onlyWhen', names)
            .every(CheckOnlyWhenCondition({ card, specForPlayer, target }));
    }

    function onlyWhenNot(names, { card, specForPlayer, target }) {
        return conditionsTypeAndNames('onlyWhen', names)
            .every(Negate(CheckOnlyWhenCondition({ card, specForPlayer, target })));
    }
    ...
    function CheckOnlyWhenCondition({ card, specForPlayer, target }) {
        return condition => {
            if (condition.targetIsOpponentCard()) {
                ...
                return condition.check({ card, specForPlayer, targetOpponentCard });
            }
            else {
    ...
    function conditionsTypeAndNames(type, conditionNames) {
        return requirementConditionsByType[type]
            .filter(condition => conditionNames.some(name => condition.is(name)));
    }
};
