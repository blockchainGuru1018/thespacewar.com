...
function StateAsserter(getState) {
  ...
  function hasStationCardInRow(cardId, rowName) {
    assertArrayOnStateHasSome(
      "stationCards",
      (stationCard) => {
        return stationCard.id === cardId && stationCard.place === rowName;
      },
      (state) => {
  ...
  function hasPutDownCardEvent({ cardId }) {
    ...
    const matchingEvents = state.events.filter(
      (event) => event.type === PutDownCardEvent.Type && event.cardId === cardId
    );
    assert(
  ...
  function hasEventBeforeOtherEvent(firstEvent, secondEvent) {
    ...
    const indexOfFirstEvent = state.events.findIndex(
      (event) =>
        event.type === firstEvent.type && event.cardId === firstEvent.cardId
    );
    const indexOfSecondEvent = state.events.findIndex(
      (event) =>
        event.type === secondEvent.type && event.cardId === secondEvent.cardId
    );
    assert(
