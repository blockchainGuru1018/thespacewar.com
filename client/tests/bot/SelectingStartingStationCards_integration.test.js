...
describe('In match mode for "select starting station cards"', () => {
    ...
    test("should put down the second starting station card in the action-row", async () => {
        const { matchController } = await setupFromState({
            ...
            currentPlayer: BotId,
            playerOrder: [BotId, PlayerId],
            stationCards: [unflippedStationCard("S1A", "action")],
            cardsOnHand: [createCard({ id: "C2A" })],
        });

        expect(matchController.emit).toBeCalledWith(
            "selectStartingStationCard",
            { cardId: "C2A", location: "action" }
        );
    });
    ...
    test("When has NO more station cards to select should select a commander", async () => {
        ...
        expect(matchController.emit).toBeCalledWith("selectCommander", {
