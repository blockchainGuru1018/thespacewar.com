...
describe("with cards from 2 sources", () => {
    ...
    test("should show card that caused requirement", async () => {
        ...
        const requirementCard = document.querySelector(
            ".findCard-requirementCard img"
        );
        const src = requirementCard.src;
        assert(
            src.includes("17"),
            `Card has src "${src}", should include its commond if of "17"`
        );
    });
...
describe("when select a card and requirement is of count 2", () => {
    test("should NOT emit action", async () => {
        await setupWithRequirement(
            requirementWithCountTwoAndTwoCardsLeft("C1A", "C2A")
        );
...
function requirementWithCountTwoAndTwoCardsLeft(
    firstCardLeftId,
    secondCardLeftId
) {
    return {
        ...
        cardGroups: [
            ...
            {
                source: "discardPile",
                cards: [createCard({ id: secondCardLeftId })],
            },
        ],
        cardCommonId: "17",
    };
...
function requirementWithTwoSourcesAndCommonId(commonId) {
    return {
        ...
        cardGroups: [
            ...
            {
                source: "discardPile",
                cards: [createCard({ id: "C2A" }), createCard({ id: "C3A" })],
            },
        ],
