<template>
  <div v-if="visible" class="debugMenu">
    <button class="escapeMenu-option" @click="showLog">
      Master log
    </button>
    <button class="escapeMenu-option" @click="showCheatOptions">
      Cheats
    </button>
    <button v-if="audioMuted" class="escapeMenu-option" @click="unmuteAudio">
      Un-mute audio
    </button>
    <button v-else class="escapeMenu-option" @click="muteAudio">
      Mute audio
    </button>
    <button class="escapeMenu-option" @click="saveMatch">
      Save state
    </button>
    <button class="escapeMenu-option" @click="restoreSavedMatch">
      Restored saved match
    </button>
    <button class="escapeMenu-option" @click="showMainMenu">
      Back
    </button>
...
module.exports = {
  ...
  computed: {
    ...escapeMenuHelpers.mapState(["view"]),
    visible() {
  ...
  methods: {
    ...
    ...debugHelpers.mapActions(["saveMatch", "restoreSavedMatch"]),
    showLog() {
    ...
    async _danger_restart_danger_() {
      const confirmed = confirm(
        "WARNING!   REALLY RESTART?   THIS IS DANGEROUS."
      );
      if (confirmed) {
        const superConfirmed = confirm("REALLY REALLY SURE?");
        if (superConfirmed) {
          const response = await ajax.jsonPost("/restart", {
