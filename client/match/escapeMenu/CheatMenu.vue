<template>
  <div v-if="visible" class="cheatMenu">
    ...
    <div class="escapeMenu-option">
      <label>
        ...
        <select v-model="cheatPlayerId">
          <option
            v-for="(value, key) in playerOrder"
            :key="key"
            :value="value"
            >{{ value }}</option
          >
    ...
    <button class="escapeMenu-option" @click="sendCheat">
      Send
    </button>
...
module.exports = {
  ...
  computed: {
    ...
    ...matchHelpers.mapGetters(["cardDataAssembler"]),
    ...matchHelpers.mapState(["playerOrder"]),
    visible() {
    ...
    cheatTypeOptions() {
      return [
        ...
        {
          value: "addDamageStationCardRequirement",
          text: "Add damage station card requirement",
        },
        {
          value: "getCardsInDeck",
          text: "Get cards in deck (result is logged in the console)",
        },
      ];
    },
    cardOptions() {
