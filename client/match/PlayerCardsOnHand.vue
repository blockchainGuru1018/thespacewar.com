...
module.exports = {
  ...
  computed: {
    ...
    playerVisibleCardsOnHandNotIncludingHoldingCard() {
      //TODO Fix naming scheme as "visible" should REALLY mean visible and not what it is now.
    ...
    visibleCardHoverActivators() {
      ...
      return this.playerVisibleCardsOnHand.map((card, index) => ({
        card,
        index,
      }));
    },
  ...
  methods: {
    ...
    documentTouchMove(event) {
      ...
      if (this.hoveringOverCardAtIndex >= 0) {
        if (event.touches[0].clientY < this.touchStartY - DragLooseThreshold) {
          this.$emit(
            "cardClick",
            this.playerVisibleCardsOnHand[this.hoveringOverCardAtIndex]
          );
          this.hoveringOverCardAtIndex = -1;
          return;
        }
      ...
      const activator = this.$refs.cardHoverActivator.find(
        (activator) => activator === touchedElement
      );
      if (activator) {
    ...
    documentMouseMove(event) {
      ...
      const yPositionAboveThreshold =
        event.clientY < this.startMouseY - DragLooseThreshold;
      const xPositionAboveThreshold =
        event.clientX < this.startMouseX - DragLooseThreshold ||
        event.clientX > this.startMouseX + DragLooseThreshold;
      if (
        !this.holdingCard &&
        (yPositionAboveThreshold || xPositionAboveThreshold)
      ) {
        this.$emit(
          "cardDrag",
          this.playerVisibleCardsOnHand[this.draggingCardAtIndex]
        );
        this.hoveringOverCardAtIndex = -1;
